    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" class="fileList">
        <el-card shadow="hover">
          ...
          <!-- 添加上传文件提示区域 -->
          <el-upload class="upload-hint" :action="`http://localhost:8002/landing/upload`" multiple="true" :limit="10" :on-success="handleUploadSuccess" :accept="'.png,.jpg,.jpeg,.pdf'" :before-upload="beforeUpload" :show-file-list="false" :data="{ uploadDir: this.uploadDirectory }" :auto-upload="true">  
  ...
  methods: {
    ...
    async confirmAutoExtraction(file_id, extracteddefaultfield) {
      ...
      this.$confirm('确认提交当前的字段抽取结果吗？', '提示', {
        ...
        try {
          const res = await axios.post('http://localhost:8002/landing/save-standard-field', {
