    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" class="fileList">
        <el-card shadow="hover">
          ...
          <!-- 添加上传文件提示区域 -->
          <el-upload class="upload-hint" :action="`http://localhost:8002/landing/upload`" multiple="true" :limit="10" :on-success="handleUploadSuccess" :accept="'.png,.jpg,.jpeg'" :before-upload="beforeUpload" :show-file-list="false" :data="{ uploadDir: this.uploadDirectory }" :auto-upload="true">  
      ...
      <!-- 中间发票预览 -->
      <el-col :span="10">
        <el-card shadow="hover" class="landing-preview" style="display: flex; align-items: center; justify-content: center;">
      ...
      <!-- 右侧字段提取 -->
      <el-col :span="10">
        <el-card shadow="hover" class="landing-extract">
          <div slot="header">
...
export default {
  ...
  data () {
    return {
      ...
      3. 如果某个字段包含子项，请使用嵌套的 JSON 结构展示；
      4. 不需要包含位置信息、类型或其他元数据；
      5. 注意：请只输出 JSON，不要添加任何解释或文本说明，也不要使用 Markdown 包裹！
      ...
      fields: [
        { fieldName: '单据种类', fieldType: '提单的标题，例如 "BILL OF LOADING"' },
        { fieldName: '提单号', fieldType: '例如 "023746123"' },
        { fieldName: '收货人信息', fieldType: '例如 "SUZHOU Company"' },
        { fieldName: '发货人信息', fieldType: '例如 "SUZHOU Company"' },
        { fieldName: '通知方', fieldType: '例如 "SUZHOU Company"' },
        { fieldName: '运输方式', fieldType: '例如 "OCEAN VESSEL"' },
        { fieldName: '船只信息和编号', fieldType: '例如 "CHENGXING 3"' },
        { fieldName: '船舶航次信息', fieldType: '例如 "H876N"' },
        { fieldName: '装货港', fieldType: '例如 "Shanghai"' },
        { fieldName: '卸货港', fieldType: '例如 "Suzhou"' },
        {
          fieldName: "货物信息",
          fieldType: "如果有多条货物信息，用数组存储，每条货物信息是一个对象",
          nestedFields: [
            { fieldName: "货物名称", fieldType: "" },
            { fieldName: "重量", fieldType: "" },
            { fieldName: "货物体积", fieldType: "" },
          ],
        },
        { fieldName: '运费', fieldType: '' },
        { fieldName: '装船日期', fieldType: '例如 "20240101"' },
        { fieldName: '签发地点与日期', fieldType: '例如 "20240101 Shanghai'}
      ...
      extractMode: 'auto',
      currentLandingImage: '',
      uploadDirectory: '../backend/upload/提单',
  ...
  methods: {
    async handleSelect(key) {
      ...
      if (this.selectedFile) {
        if (this.extractMode === 'auto' || this.extractMode === "result") {
          ...
          const res = await axios.get(`http://localhost:8002/ladning/extracted-field/${idx}`);
          ...
          const res = await axios.get(`http://localhost:8002/landing/extracted-custom-field/${idx}`);
          if (res.data.extracted_data) {
      ...
      if (checked) {
        this.$confirm('确定要清空所有文件吗？', '提示', {
          ...
          try {
            ...
            // 调用后端接口删除服务器上所有文件
            axios.post('http://localhost:8002/landing/clear-files', {
    ...
    async deleteFile(file_name, file_id) {
      this.$confirm('确认删除此文件吗？', '提示', {
        ...
        console.log(file_id)
        try {
          await axios.post('http://localhost:8002/landing/delete-file', {
            filename: file_name,
            uploadDir: this.uploadDirectory,
            file_id: file_id
            // 你可以从 data() 获取这个路径
    ...
    async fetchFiles() {
      const res = await fetch('http://localhost:8002/landing/files');
    ...
    async fetchExtractedData(nowPrompt, fileObj) {
      ...
      try {        
        if (this.extractMode === "auto") {
          ...
          const res = await axios.post('http://localhost:8002/landing/open-loading', {
        ...
        // 更新表格数据
        if (this.extractMode === "auto") {
          const res = await axios.post('http://localhost:8002/landing/save-extracted', {
            file_id: Number(fileObj.id),
          ...
          const res = await axios.post('http://localhost:8002/landing/save-custom-extracted',{
            file_id: Number(fileObj.id),
            extracted_data: this.parseJsonToTable(parsed)
          }, {
            headers: {
        ...
        fileObj.loadingText = '';
        const res = await axios.post('http://localhost:8002/landing/close-loading', {
            file_id: Number(fileObj.id),
          }, {
            headers: {
              'Content-Type': 'application/json'
    ...
    async handleExtractModeChange(mode, fileObj=null) {
      ...
      if (this.extractMode === "auto") {
        const res = await axios.get(`http://localhost:8002/landing/extracted-field/${idx}`);
        if (res.data.extracted_data) {
          fileObj.ExtractedDefaultField = res.data.extracted_data;
        } else {
          await this.fetchExtractedData(this.LandingPrompt, fileObj);
    ...
    reExtract() {
      if (this.extractMode === 'auto') {
        ...
        this.fetchExtractedData(this.LandingPrompt, this.selectedFile)
      } else if (this.extractMode === 'custom') {
    ...
    async customUploadRequest({ file }) {
      ...
      await axios.post('http://localhost:8002/landing/upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      }).then(response => {
