    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" class="fileList">
        <el-card shadow="hover">
          ...
          <!-- 添加上传文件提示区域 -->
          <el-upload class="upload-hint" :action="`http://localhost:8002/invoice/upload`" multiple="true" :limit="10" :on-success="handleUploadSuccess" :accept="'.png,.jpg,.jpeg,.pdf'" :before-upload="beforeUpload" :show-file-list="false" :data="{ uploadDir: this.uploadDirectory }" :auto-upload="true">  
      ...
      <!-- 中间发票预览 -->
      <el-col :span="10">
        <el-card shadow="hover" class="invoice-preview" style="display: flex; align-items: center; justify-content: center;">
      ...
      <!-- 右侧字段提取 -->
      <el-col :span="10">
        <el-card shadow="hover" class="invoice-extract">
          <div slot="header">
...
export default {
  ...
  data () {
    return {
      ...
      extractMode: 'auto',
      currentInvoiceImage: '',
      uploadDirectory: '../backend/upload/发票',
  ...
  methods: {
    async handleSelect(key) {
      ...
      if (this.selectedFile) {
        this.currentInvoiceImage = this.selectedFile.thumbnail;
    ...
    async handleExtractModeChange(mode, fileObj=null) {
      ...
      if (this.extractMode === "auto") {
        ...
        if (res.data.extracted_data) {
          ...
          await this.fetchExtractedData(this.InvoicePrompt, fileObj);
    ...
    reExtract() {
      if (this.extractMode === 'auto') {
        ...
        this.fetchExtractedData(this.InvoicePrompt, this.selectedFile)
      } else if (this.extractMode === 'custom') {
    ...
    exportToExcel() {
      ...
      if (this.extractMode === "auto") {
        ...
        console.log(this.selectedFile)
        extractedFields = selectedFile.ExtractedCustomField
      };
    ...
    async loadStandardJson(selectedFile=null) {
      try {
        ...
        const extractedFields = this.selectedFile.extractedDefaultFields;
        // 调用后端接口进行准确率计算
      ...
      // const extractedFields = this.selectedFile.extractedDefaultFields;
      // const standardFields = this.standardFields;

      // let matchCount = 0;
      // let totalCount = 0;

      // // console.log("提取字段：", extractedFields)
      // // console.log("标准字段：", standardFields)
      // // 构建标准字段的路径 -> field 映射
      // const standardMap = new Map();

      // for (const field of standardFields) {
      //   const path = this.buildFieldPath(standardFields, field);
      //   standardMap.set(path, field);
      ...
      //   const path = this.buildFieldPath(extractedFields, field);

      //   if (standardMap.has(path)) {
      //     const standardField = standardMap.get(path);
      //     const extractedValue = (field.fieldValue || '').trim();
      //     const standardValue = (standardField.fieldValue || '').trim();

      //     totalCount += 1;

      //     if (extractedValue === standardValue) {
      //       matchCount += 1;
      //     } else {
      //       console.log(`不匹配字段: ${path} | 提取值: "${extractedValue}" | 标准值: "${standardValue}"`);
      //     }
      //   } else {
      //     console.log(`未找到匹配的标准字段路径: ${path}`);
      //   }
      // }

      // const accuracyRate = totalCount > 0 ? ((matchCount / totalCount) * 100).toFixed(2) : '0.00';

      // console.log(`共对比字段数量: ${totalCount}`);
      // console.log(`匹配字段数量: ${matchCount}`);
      // console.log(`识别准确率: ${accuracyRate}%`);
    },
