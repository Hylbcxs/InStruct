    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" class="fileList">
        <el-card shadow="hover">
          ...
          <!-- 添加上传文件提示区域 -->
          <el-upload class="upload-hint" :action="`http://localhost:8002/declaration/upload`" multiple="true" :limit="10" :on-success="handleUploadSuccess" :accept="'.png,.jpg,.jpeg'" :before-upload="beforeUpload" :show-file-list="false" :data="{ uploadDir: this.uploadDirectory }" :auto-upload="true">  
...
export default {
  ...
  data () {
    return {
      DeclarationPrompt: `
        ...
        ### 需要提取的字段：
        - **单据种类**：报关单抬头，例如 "中华人民共和国海关出口货物报关单"
        - **合同协议号**：例如 "023746123"
        - **申报日期**：例如 "20200101"
        - **发货人信息**
          - 名称：例如 "苏州科技有限公司"
      ...
      fields: [
        ...
        { fieldName: '发货人信息', 
          ...
          nestedFields: [
            ...
            {fieldName: '编号', fieldType: '例如 "12334857632"'}
          ]
        },
        { fieldName: '收货人信息', 
          fieldType: '',
  ...
  methods: {
    ...
    exportToExcel() {
      let extractedFields = '';
      if (this.extractMode === "auto") {
        extractedFields = this.selectedFile.extractedDefaultFields
      } else if (this.extractMode === "custom") {
