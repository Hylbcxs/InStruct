...
@router.post("/extract")
async def extract_fields(payload: Dict[str, Any]):
    ...
    try:
        ...
        result = await loop.run_in_executor(
            executor, 
            ai_service.extract_fields_from_image,
            image_url,
            prompt,
            use_ocr
        )
...
@router.post("/identify-document-type")
async def identify_document_type(payload: Dict[str, Any]):
    loop = asyncio.get_running_loop()
    try:
        image_url = payload.get("image_url", "")
        ...
        document_type = await loop.run_in_executor(
            executor,
            ai_service.identify_document_type,
            image_url
        )
